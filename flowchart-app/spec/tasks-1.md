...
### Implementation
- [ ] Scaffold project with `npm create vite@latest flowchart-app -- --template react-ts`
- [ ] Install runtime dependencies: `@xyflow/react`, `@reactflow/node-resizer`, `mermaid`
...
- [ ] Add `test` config to `vite.config.ts`: `environment: 'jsdom'`, `globals: true`, `setupFiles: ['./src/setupTests.ts']`
- [ ] Create `src/setupTests.ts` importing `@testing-library/jest-dom`
- [ ] Create `src/types/diagram.ts` with placeholder export (filled in Task 2)

### Tests
- [ ] Verify `npm install` completes without errors
- [ ] Verify `npm test` runs without configuration errors (zero tests passing is fine)
...
### Implementation
- [ ] Define types in `src/types/diagram.ts`:
  - `type ShapeType = 'process' | 'decision' | 'terminator' | 'io'`
  - `interface FlowNodeData { label: string; shapeType: ShapeType }`
- [ ] Create `src/utils/mermaidGenerator.ts` exporting `generateMermaid(nodes, edges): string`
- [ ] Map each ShapeType to Mermaid node syntax:
  - `process` → `id["label"]`
  - `decision` → `id{"label"}`
  - `terminator` → `id(["label"])`
  - `io` → `id[/"label"/]`
- [ ] Map each edge to `sourceId --> targetId`
- [ ] Output starts with `flowchart TD\n`
- [ ] Return empty diagram string `flowchart TD\n` when nodes array is empty

### Tests
- [ ] Write `src/utils/mermaidGenerator.test.ts`: unit tests for generator → verifies US-5
...
### Implementation
- [ ] Create `src/components/FlowCanvas/nodes/ProcessNode.tsx`:
...
- [ ] Repeat the same pattern for `DecisionNode.tsx` (diamond — SVG polygon), `TerminatorNode.tsx` (oval — SVG ellipse), `IONode.tsx` (parallelogram — SVG polygon)
- [ ] Export `nodeTypes` object from `src/components/FlowCanvas/nodes/index.ts`: `{ process: ProcessNode, decision: DecisionNode, terminator: TerminatorNode, io: IONode }`

### Tests
- [ ] Write `src/components/FlowCanvas/nodes/ProcessNode.test.tsx` → verifies US-2, US-3
...
- [ ] Write equivalent smoke-render tests for `DecisionNode`, `TerminatorNode`, `IONode` (renders with label, double-click shows input)
- [ ] Run `npm test` — all tests pass
...
### Implementation
- [ ] Create `src/components/ShapeSidebar/DraggableShape.tsx`:
...
- [ ] Create `src/components/ShapeSidebar/ShapeSidebar.tsx`:
  - Renders 4 `<DraggableShape>` tiles: process, decision, terminator, io
- [ ] Create `src/components/FlowCanvas/FlowCanvas.tsx`:
...
### Tests
- [ ] Write `src/components/ShapeSidebar/ShapeSidebar.test.tsx` → verifies US-1
...
- [ ] Write `src/components/FlowCanvas/FlowCanvas.test.tsx` → verifies US-1
...
### Implementation
- [ ] In `FlowCanvas.tsx`, pass `onConnect` handler to `<ReactFlow>`:
  - Uses `addEdge` from `@xyflow/react` to append the new edge to edges state
- [ ] Confirm `<Handle>` components in each node (added in Task 3) have correct `type="source"` and `type="target"` on all four sides so any port can initiate or receive a connection
- [ ] Set `connectionMode={ConnectionMode.Loose}` on `<ReactFlow>` so any handle can connect to any other handle
- [ ] Verify connectors re-route automatically (React Flow default behaviour — no extra work needed)

### Tests
- [ ] Write additional cases in `src/components/FlowCanvas/FlowCanvas.test.tsx` → verifies US-4
...
### Implementation
- [ ] Create `src/components/OutputPanel/MermaidPreview.tsx`:
...
- [ ] Create `src/components/OutputPanel/OutputPanel.tsx`:
  - Accepts `mermaidText: string` prop
...
### Tests
- [ ] Write `src/components/OutputPanel/OutputPanel.test.tsx` → verifies US-5, US-6
...
- [ ] Apply CSS (via `src/App.css`) for three-column layout: sidebar fixed width (~160px), canvas flex-grow, output panel fixed width (~360px)
- [ ] Wrap `<FlowCanvas>` in `<ReactFlowProvider>` so `useReactFlow()` hooks work inside child components

### Tests
- [ ] Write `src/App.test.tsx` → integration test verifying US-1, US-5, US-6
